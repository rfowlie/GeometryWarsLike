...
public class SpawnPattern
{
    ...
    private void Execute()
    {
        transform.rotation = Quaternion.FromToRotation(transform.up, transform.position - target.position) * transform.rotation;
        points = del();
        p.Clear();
        ...
        {
            points[i] += transform.position;
            Vector3 dir = (target.position - points[i]).normalized;
            //Debug.Log("DrawLines");
            Debug.DrawLine(points[i], points[i] + dir, Color.yellow);
            if (Physics.Raycast(points[i], dir, out hit, float.PositiveInfinity))
            {
